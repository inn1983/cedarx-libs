...
cedarv_decoder_t* libcedarv_init(s32* return_value)
{
	...
	*return_value = CEDARV_RESULT_ERR_FAIL;
	...
	p->config_info.yv12_output_enable=1;
	p->config_info.yv32_output_enable=0;
	p->config_info.decode_no_delay=0;
	p->config_info.yv32_transform_on_yv12=0;
        p->config_info.nDisplayFrameRequestMode=0;   //0:malloc self; 1:from gui.
        p->config_info.ts_stream_source_type=0; 
...
static s32 vdecoder_open(cedarv_decoder_t* p)
{
	...
	LogX(L2, "    aspec_ration = %d, is_pts_correct = %d", decoder->stream_info.aspect_ratio, decoder->stream_info.is_pts_correct);
...
printf(" free rotate framebuffer\n");
...
printf(" alloc rotate framebuffer\n");
...
static s32 vdecoder_set_video_bitstream_info(cedarv_decoder_t* p, cedarv_stream_info_t* info)
{
	...
	printf("set vstream info bitstream\n");
	...
	//decoder->stream_info.frame_rate       = 30*1000;//info->frame_rate;
	...
	//decoder->stream_info.init_data_len    = 0;//info->init_data_len;
	...
	if(decoder->stream_info.init_data_len > 0)
	{
...
static s32 set_cedarv_stream_info(cedarv_stream_info_t *cedarv_stream_info, vstream_info_t *vstream_info)
{
    ...
    cedarv_stream_info->aspect_ratio     = vstream_info->aspect_ratio;
